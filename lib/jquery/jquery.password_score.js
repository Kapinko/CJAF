...
(function ($, window) {
	...
	defaults		= {
		 ...
		 */
		"scoreUpdate": function (score, inst) {
			return false;
		},
		/**
	...
	PasswordScore.prototype	= {
		...
		"getScore": function () {
			...
			deductions	= this.getTotalDeductions(), total;
		...
		"getLengthBonus": function () {
			...
			if (this._isBonusEligible(length, 'length')) {
				bonus	= length * (this.options.lengthBonusMultiplier || 1);
			}
			return bonus;
		},
		/**
		...
		"getAlphaUCBonus": function () {
			...
			if (this._isBonusEligible(count, 'alpha')) {
				bonus	= parseInt((this.getLength() - count) * (this.options.alphaBonusMultiplier || 1), 10);
			}
			return bonus;
		},
		/**
		...
		"getNumericBonus": function () {
			...
			if (this._isBonusEligible(count, 'numeric')) {
				bonus		= parseInt(count * (this.options.numericBonusMultiplier || 1), 10);
			}
			return bonus;
		},
		/**
		...
		"getSymbolicBonus": function () {
			...
			if (this._isBonusEligible(count, 'symbolic')) {
				bonus		= parseInt(count * (this.options.symbolicBonusMultiplier || 1), 10);
			}
			return bonus;
		},
		/**
		...
		"getMiddleBonus": function () {
			...
			if (this._isBonusEligible(count, 'middle')) {
				bonus		= parseInt(count * (this.options.middleBonusMultiplier || 1), 10);
			}
			return bonus;
		},
		/**
		...
		"getComplexityBonus": function () {
			...
			if (this._isBonusEligible(complexity, 'complexity')) {
				bonus	= complexity * (this.options.complexityBonusMultiplier || 1);
			}
			return bonus;
		},
		/**
		...
		"getConsecAlphaUCDeduction": function () {
			return this.consecutiveAlphaUC * (this.options.consecAlphaDeductionMultiplier || 1);
		},
		/**
		...
		"getConsecAlphaLCDeduction": function () {
			return this.consecutiveAlphaLC * (this.options.consecAlphaDeductionMultiplier || 1);
		},
		/**
		...
		"getConsecNumericDeduction": function () {
			return this.consecutiveNumeric * (this.options.consecNumericDeductionMultiplier || 1);
		},
		/**
		...
		"getSeqAlphaDeduction": function () {
			return this.sequentialAlpha * (this.options.seqAlphaDeductionMultiplier || 1);
		},
		/**
		...
		"getSeqNumericDeduction": function () {
			return this.sequentialNumeric * (this.options.seqNumericDeductionMultiplier || 1);
		},
		/**
		...
		"getSeqSymbolicDeduction": function () {
			return this.sequentialSymbolic * (this.options.seqSymbolicDeductionMultiplier || 1);
		},
		/**
		...
		"gotRepeat": function (index, repeatIndexList) {
			...
			for (i = 0; i < repeatIndexList.length; i += 1) {
				/*
		...
		_isBonusEligible: function (count, type) {
			var is_eligible	= false, minimum_score;
			...
			case 'complexity':
				minimum_score	= this.options.minimumComplexityScore;
				if (this.getLength() >= this.options.minimumLength) {
					minimum_score -= 1;
				}
	...
	$.extend(PasswordScorer.prototype, {
		...
		_checkForConsecutiveCharacterPatterns: function (password) {
			var passAsArray		= password.replace(/\s+/g, "").split(/\s*/), index, current;

			for (index = 0; index < passAsArray.length; index += 1) {
				current	= passAsArray[index];
		...
		_checkForDuplicateCharacter: function (passwordAsArray, character, characterIndex) {
			var dupFound	= false,
				indexList	= [], index, dupCheck;

			for (index = 0; index < passwordAsArray.length; index += 1) {
				dupCheck	= passwordAsArray[index];
		...
		_checkForAlphaStringPatterns: function (password) {
			var boundry	= ALPHAS.length - 3, i, pattern;

			for (i = 0; i < boundry; i += 1) {
				pattern = ALPHAS.substring(i, parseInt(i + 3, 10));
		...
		_checkForNumericStringPatterns: function (password) {
			var boundry	= NUMERICS.length - 3, i, pattern;
			for (i = 0; i < boundry; i += 1) {
				pattern	= NUMERICS.substring(i, parseInt(i + 3, 10));
				if (this._hasPattern(pattern, password)) {
		...
		_checkForSymbolicStringPatterns: function (password) {
			var boundry	= SYMBOLS.length - 3, i, pattern;
			for (i = 0; i < boundry; i += 1) {
				pattern	= SYMBOLS.substring(i, parseInt(i + 3, 10));
				if (this._hasPattern(pattern, password)) {
	...
	$.extend(PasswordScoreInstance.prototype, {
		...
		_handleKeyUp: function (event) {
			...
			handler		= this.config.scoreUpdate;
	...
	$.extend(PasswordScorePlugin.prototype, {
		...
		"_getScorePasswordScore": function (target) {
			var inst	= this._getInstance(target), score;
			score	= inst.score();
		...
		_newInstance: function (target, settings) {
			var id		= target[0].id.replace(/([:\[\]\.])/g, '\\\\$1'), inst; // escape jQuery meta chars
			inst	= new PasswordScoreInstance(id, target, settings);
			return inst;
		},
		/**
	...
	$.fn.passwordScore	= function (options) {
		...
		this.each(function () {
			if (typeof options === 'string') {
				...
				score	= $.passwordScore._attachInstance(this, options);
			}
		});
		return score;
	};
