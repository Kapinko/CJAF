(function($){
	...
	var AuthConfig	= function(options){
		...
		if(typeof this.config.timestampFunction !== 'function'){
			this.config.timestampFunction	= function(){
				return new Date().getTime();
			};
		}

		if(typeof this.config.queryResponseParser !== 'function'){
	...
	AuthInstance.prototype = {
		...
		'_isLoggedIn': function(){
			...
			if(this.hasCacheExpired()){
				var parser	= $.proxy(config, "parseQueryResponse");
				...
				.done($.proxy(function (response, textStatus, XMLHttpRequest) {
					this.user		= parser(response);
					this.cacheTime	= config.getTimestamp() + config.getCacheTimeout();
				}, this))
				.error($.proxy(function (XMLHttpRequest, textStatus, exception) {
